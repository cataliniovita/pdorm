...
func main() {
    interpolate := os.Getenv("INTERPOLATE") == "1"
    db, err := sql.Open("mysql", dsn(interpolate))
    if err != nil { log.Fatal(err) }
    if err := db.Ping(); err != nil { log.Fatal(err) }

    http.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
        if err := db.Ping(); err != nil { jsonWrite(w, 500, resp{"ok": false, "error": err.Error()}); return }
        jsonWrite(w, 200, resp{"ok": true, "interpolate": interpolate})
    })
    ...
    http.HandleFunc("/vuln", func(w http.ResponseWriter, r *http.Request) {
        ...
        col := r.URL.Query().Get("col")
        if col == "" { col = "name" }
        // VULN: naive backtick escaping for identifiers; with interpolateParams=true the client may mis-handle '?'
        sanitized := replaceBackticks(col)
        q := fmt.Sprintf("SELECT `%s` AS val FROM fruit WHERE name = ?", sanitized)
        rows, err := db.Query(q, name)
        if err != nil { jsonWrite(w, 500, resp{"mode": map[string]bool{"interpolate": interpolate}, "query": q, "error": err.Error()}); return }
        var out []map[string]any
        for rows.Next() { var v sql.NullString; _ = rows.Scan(&v); out = append(out, map[string]any{"val": v.String}) }
        jsonWrite(w, 200, resp{"mode": map[string]bool{"interpolate": interpolate}, "query": q, "rows": out})
    })

    log.Println("go-mysql-emulate listening on :8080")
